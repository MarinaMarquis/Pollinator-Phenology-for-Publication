# Supplemental Analysis: GAM for Pollinator Flight Period
...
summary(fp_rel$pop_den)
sd(fp_rel$pop_den)

# Plot it
ggplot(fp_rel, aes(x = pop_den)) +
  geom_histogram(bins = 30, fill = "steelblue", color = "white") +
  theme_classic() +
  labs(x = "Population Density", y = "Count",
       title = "Distribution of Population Density in the data set")
# Looks like there is a skew towards lower population densities 

# Summarize by species 
fp_data_summary <- fp_data %>%
  group_by(species) %>%
  summarise(min_GHMI = min(pop_den, na.rm = TRUE),
            max_GHMI = max(pop_den, na.rm = TRUE),
            n = n()) %>%
  arrange(min_GHMI) %>%
  print(n = 50)
...
check_species <- function(df) {
  m <- gam(duration ~ s(pop_den, k = min(5, length(unique(df$pop_den)))),
           data = df, method = "REML")
  tibble(
    n = nrow(df),
    range = diff(range(df$pop_den)),
    sd = sd(df$pop_den),
    edf = summary(m)$edf[1]  
  )
}
...
# Quick visualization 
ggplot(fp_data, aes(x = pop_den)) +
  geom_histogram(bins = 30, fill = "steelblue", color = "white") +
  theme_classic(base_size=14) +
  labs(x = "Population Density", y = "Count",
       title = "Distribution of Population Density in the Data Set After Filtering")

# Save it 
ggsave("Figures/distribution_of_pop_den_values_in_GAM_dataset.png", width=6, height=6, units="in")
...
data_for_models_summary <- fp_data %>%
  ...
  summarise(
    ...
    min_GHMI = min(pop_den, na.rm = TRUE),
    max_GHMI = max(pop_den, na.rm = TRUE),
...
write.csv(data_for_models_summary, "Data/data_for_models_summary_pop_den.csv")
...
#The population seems to explain very little deviance in the model, species and lat/long explain much 
...
print(aic_full_dur)
#population density only marginally improves the model
...
# Now add pop_den
gam_1_bi <- gam(duration ~ pop_den + + s(temp) + s(prcp) +
                  s(lat, lon, bs="tp"), 
                family = gaussian(),
                method = "REML",
                data=Bombus_impatiens)
...
AICc(gam_null_bi)
AICc(gam_1_bi)
#So in this case the adding population improved model fit, but only slightly 
...
#The null model preforms better  
...
# Now add pop_den
gam_1_bi_on <- gam(onset ~ pop_den + s(temp) + s(prcp) +
                     s(lat, lon, bs="tp"), 
                   family = gaussian(),
                   method = "REML",
                   data=Bombus_impatiens)
...
#While pop_den is not significant, it does improve deviance explained

# Now let's see how they rank
AICc(gam_null_bi_on)
AICc(gam_1_bi_on)
#Adding population density decreases model fit 
...
delta_aic_off
#Adding population density strongly improves model fit 
...
# Now add pop_den
gam_1_bi_off <- gam(offset ~ pop_den + s(temp) + s(prcp) +
                      s(lat, lon, bs="tp"), 
                    family = gaussian(),
                    method = "REML",
                    data=Bombus_impatiens)
...
saveRDS(species_gam_full, "Data/GAM_results/species_gam_full_pop_den.rds")
...
C_albipunctatus_data <- fp_data %>%
  ...
  summarise(range = diff(range(pop_den)),
            sd    = sd(pop_den))%>%
  print()
...
m_no1 <- gam(duration ~ pop_den + s(lat, lon, k = k_val, bs = "tp"),
             family = gaussian(),
             method = "REML",
...
plot_model_group <- function(species_vec, model_name, ncol = 3) {
  plots <- lapply(species_vec, function(sp) {
    ...
    ghmi_vals <- tibble(
      pop_den = seq(0, 1, length.out = 100),
    ...
    # Plot manually
    ggplot(plot_df, aes(x = pop_den, y = fit)) +
...
# Save the plots 
ggsave("Figures/GAM_onset_species_pop_den.png", plot_model_group(onset_species, "onset"), 
       width = 8, height = 8, dpi = 300)

ggsave("Figures/GAM_offset_species_pop_den.png", plot_model_group(offset_species, "offset"), 
       width = 8, height = 8, dpi = 300)

ggsave("Figures/GAM_duration_species_pop_den.png", plot_model_group(duration_species, "duration"), 
