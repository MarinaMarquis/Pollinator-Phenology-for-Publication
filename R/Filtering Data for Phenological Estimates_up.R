...
observation_count_check <- filtered_other %>%
  group_by(grid_id, species) %>%
  summarize(observation_count = n(), .groups = 'drop')
#
...
# Make sure the date column is formatted correctly for analysis 
...
length(unique(filtered_other$species))
...
# Export the rds file 
saveRDS(filtered_5, "Data/filtered_5_up.rds")
